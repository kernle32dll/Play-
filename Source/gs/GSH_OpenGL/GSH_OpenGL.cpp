...
void CGSH_OpenGL::SetupFramebuffer(uint64 frameReg, uint64 zbufReg, uint64 scissorReg, uint64 testReg)
{
	...
	if(m_renderState.framebufferHandle != 0 && m_renderState.framebufferHandle != framebuffer->m_framebuffer)
	{
		...
		if(framebufferIterator != std::end(m_framebuffers))
		{
			...
			{
                auto imgbuffer = FindOrCreateBitmap(framebuffer);
...
CGSH_OpenGL::BitmapPtr CGSH_OpenGL::FindOrCreateBitmap(const FramebufferPtr& framebuffer)
{
	auto bitmapIterator = std::find_if(std::begin(m_bitmaps), std::end(m_bitmaps),
	                                   [&](const BitmapPtr& bitmap)
	                                   {
		                                   return (
		                                       framebuffer->m_width * m_fbScale == bitmap->GetWidth() &&
		                                       framebuffer->m_height * m_fbScale == bitmap->GetHeight() &&
	...
	auto sharedPtr = std::make_shared<Framework::CBitmap>(framebuffer->m_width * m_fbScale, framebuffer->m_height * m_fbScale, 32);
